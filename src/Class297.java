...
public class Class297 {
	...
	boolean method2835(byte i) {
		try {
			...
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("mj.a(").append(')').toString());
		}
	}
	...
	public static int method2836(byte i) {
		try {
			...
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("mj.r(").append(')').toString());
		}
	}

	static final void decodePlayerMasks(RsBitsBuffer stream, int playerIndex, Player player, int mask) {
		try {
			byte movementType = MovementType.aMovementType_6543.type;
			if ((mask & 0x4) != 0) { // gfx 1
			...
			if ((mask & 0x40) != 0) { // face direction
				player.chatIcon = stream.readUnsignedShort() * -1530078181;
				if (2050671733 * player.moveQueueSize == 0) {
					player.method4414(player.chatIcon * 1208589843, (byte) 1);
			...
			if (0 != (mask & 0x8)) { // hits
				...
				if (i_20_ > 0) {
					for (int i_21_ = 0; i_21_ < i_20_; i_21_++) {
						...
						player.addMovementStep(i_25_, i_23_, i_22_, i_24_, 443738891 * GameClient.anInt8884, i_26_, i_27_, (byte) 121);
					}
				}
			}
			if (0 != (mask & 0x400000)) { // gfx 3
				int i_28_ = stream.readUnsignedShort128(1614800884);
				int i_29_ = stream.readUnsignedInt();
				if (i_28_ == 65535)
					i_28_ = -1;
			...
			if ((mask & 0x2000) != 0) {
				...
				player.hasOverheadString = (i_41_ & 0x8000) != 0;
				player.anInt10087 = -257621575 * (i_41_ & 0x7fff);
				player.anInt10086 = (1208636921 * GameClient.anInt8884 + -1132907677 * player.anInt10087 + 1195117671 * player.anInt10088);
			}
			if ((mask & 0x1000) != 0) { // forcemovement
				...
				player.anInt10095 = (stream.readUnsignedShort() + GameClient.anInt8884 * 443738891) * 996079737;
				player.anInt10103 = (stream.readUnsignedShort() + GameClient.anInt8884 * 443738891) * 1000906529;
				player.anInt10104 = stream.readUnsignedShort() * 1386670945;
				if (player.isTeleporting) {
					player.anInt10098 += 259583125 * player.teleportX;
					player.anInt10100 += (player.teleportY * -611622223);
					player.anInt10099 += (-1551443089 * player.teleportX);
					player.anInt10101 += (1804486437 * player.teleportY);
					player.moveQueueSize = 0;
				} else {
					...
					player.moveQueueSize = -1013322787;
				}
				player.anInt10125 = 0;
			}
			...
			if ((mask & 0x200) != 0)
				player.isClickable = stream.readUnsignedByte() == 1;
			if (0 != (mask & 0x100000)) {
			...
			if ((mask & 0x10000) != 0) {
				int i_53_ = ((stream.payload[((stream.pos += 116413311) * 385051775) - 1]) & 0xff);
				for (int i_54_ = 0; i_54_ < i_53_; i_54_++) {
					int i_55_ = stream.readUnsignedShortLE((byte) 117);
					int i_56_ = stream.readUnsignedInt();
					player.hitSplatQueue.method800(i_55_, i_56_, (byte) 1);
				}
			}
			...
			if ((mask & 0x200000) != 0) {
				...
				player.walkStyle = (byte) stream.readUnsignedByteC((short) -10460);
				player.anInt10105 = ((443738891 * GameClient.anInt8884 + stream.readUnsignedShort()) * 1828453179);
				player.anInt10106 = ((GameClient.anInt8884 * 443738891 + stream.readUnsignedShort()) * -473408095);
			}
			if ((mask & 0x80000) != 0) {
				player.hitSplatQueue.method797((byte) -63);
				int i_57_ = ((stream.payload[((stream.pos += 116413311) * 385051775) - 1]) & 0xff);
				for (int i_58_ = 0; i_58_ < i_57_; i_58_++) {
					int i_59_ = stream.readUnsignedShort128(-722350822);
					int i_60_ = stream.readIntV1(1512777839);
					player.hitSplatQueue.method800(i_59_, i_60_, (byte) 1);
				}
			}
			if (player.isTeleporting) {
				if (movementType == 127)
					player.method4472(659113193 * player.teleportX, -504032157 * player.teleportY, -2047021901);
				else {
					byte i_61_;
					if (MovementType.aMovementType_6543.type != movementType)
						i_61_ = movementType;
					else
						i_61_ = Class10.playersMovementTypes[playerIndex];
					GameContext.method5575(player, i_61_, 1037382874);
					player.resetPosition(659113193 * player.teleportX, -504032157 * player.teleportY, i_61_, -2128158189);
				}
			}
		} catch (RuntimeException runtimeexception) {
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("mj.u(").append(')').toString());
		}
	}
	...
	static final void method2838(ClientScript2 class403, int i) {
		try {
			...
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("mj.amu(").append(')').toString());
		}
	}
}
