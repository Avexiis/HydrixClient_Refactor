...
public class Class415 {
	...
	public static byte[] method5586(Object object, int i, int i_0_, byte i_1_) {
		try {
			...
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("rf.b(").append(')').toString());
		}
	}
	...
	public static int method5587(int i, int i_2_, int i_3_, int i_4_, int i_5_, int i_6_, int i_7_) {
		try {
			...
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("rf.i(").append(')').toString());
		}
	}
	...
	static String method5588(CharSequence charsequence, Class513 class513, byte i) {
		try {
			...
			int i_9_ = 0;
			int i_10_;
			for (i_10_ = charsequence.length(); i_9_ < i_10_ && VarBitType.method3980(charsequence.charAt(i_9_), 185580898); i_9_++) {
				/* empty */
			}
			for (/**/; (i_10_ > i_9_ && VarBitType.method3980(charsequence.charAt(i_10_ - 1), -1519613648)); i_10_--) {
				/* empty */
			}
			...
			for (int i_12_ = i_9_; i_12_ < i_10_; i_12_++) {
				...
				if (!IntegerPreferenceField.method5621(c, -1788323982)) {
					if (i >= 5) {
						/* empty */
					}
				} else {
