	...
	static final void decodeNPCsMasks(short i) {
		try {
			...
			for (int i_0_ = 0; i_0_ < GameClient.anInt8808 * -976358333; i_0_++) {
				...
				NPC npc = (NPC) ((LinkableObject) GameClient.npcs.get(npcIndex)).anObject7366;
				...
				if ((mask & 0x80000) != 0) { // Unidentified - ordinal 2
					...
					npc.walkStyle = (byte) stream.readUnsignedByte128(-1014855659);
				...
				if (0 != (mask & 0x4)) { // Hit ? ordinal 7
					...
					if (i_10_ > 0) {
						for (int i_11_ = 0; i_11_ < i_10_; i_11_++) {
							...
							npc.addMovementStep(i_15_, i_13_, i_12_, i_14_, GameClient.anInt8884 * 443738891, i_16_, i_17_, (byte) 6);
						}
					}
				}
				if ((mask & 0x1000) != 0) {// ordinal 8
					...
					if (null != npc.definition.modifiedModelColors) {
						i_19_ = npc.definition.modifiedModelColors.length;
					}
					int i_20_ = 0;
					if (npc.definition.modifiedTextureColors != null) {
						i_20_ = npc.definition.modifiedTextureColors.length;
					}
				...
				if ((mask & 0x400) != 0) {// ordinal 13
					...
					npc.hasOverheadString = (i_47_ & 0x8000) != 0;
				...
				if ((mask & 0x800) != 0) {// Force movement ordinal 15
					...
					npc.anInt10095 = (stream.readUnsignedShortLE((byte) -54) + 443738891 * GameClient.anInt8884) * 996079737;
					npc.anInt10103 = (stream.readUnsignedShortLE128(1478539457) + GameClient.anInt8884 * 443738891) * 1000906529;
				...
				if (0 != (mask & 0x400000)) {// ordinal 16
					...
					for (int i_53_ = 0; i_53_ < i_52_; i_53_++) {
						...
						int i_55_ = stream.readUnsignedInt();
				...
				if ((mask & 0x20000) != 0) {// ordinal 19
					...
					for (int i_63_ = 0; i_63_ < i_62_; i_63_++) {
						...
						int i_65_ = stream.readUnsignedInt();
				...
				if ((mask & 0x200000) != 0) {// ordinal 20
					...
					int i_67_ = 0;
					if (null != npc.definition.modifiedModelColors) {
						i_67_ = npc.definition.modifiedModelColors.length;
					}
					int i_68_ = 0;
					if (npc.definition.modifiedTextureColors != null) {
						i_67_ = npc.definition.modifiedTextureColors.length;
					}
			...
			throw ErrorContext.info(runtimeexception, new StringBuilder().append("tv.kd(").append(')').toString());
		}
	}
}
